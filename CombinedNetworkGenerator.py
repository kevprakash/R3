...
def generateAllNetworks(imageSize, numberOfFilters, filterSizes, latentSpaceLength, controllerNodesPerLayer, controllerOutputLength, rewardNodesPerLayer, rewardOutputLength, dropoutRate=0, optimizer='rmsprop', hiddenActivation=tf.nn.relu, activation=tf.nn.sigmoid):
    ...
    reward = RN.initializeRewardNetwork(encode, controller, latentSpaceLength=latentSpaceLength, controllerOutputLength=controllerOutputLength, nodesPerLayer=rewardNodesPerLayer, outputLength=rewardOutputLength, dropoutRate=dropoutRate, optimizer=optimizer, hiddenActivation=hiddenActivation, activation=activation)
